...
PGID=$(docker ps -qf name=callcenter_postgres 2>/dev/null | head -1)
if [ -z "$PGID" ]; then
  echo "ERRO: Postgres não iniciou. Verificando..."
  docker service ps callcenter_postgres --no-trunc
  exit 1
fi

echo "=== 3. Aguardando Postgres aceitar conexões ==="
for i in $(seq 1 12); do
  if docker exec "$PGID" pg_isready -U postgres -q 2>/dev/null; then
    echo "Postgres aceitando conexões!"
    break
  fi
...
echo "Banco criado (ou já existia)"
